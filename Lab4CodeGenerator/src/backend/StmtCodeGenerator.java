...
public class StmtCodeGenerator extends Visitor<Void> {
	...
	public Void visitBreakStmt(BreakStmt nd) {
		...
		Unit target = breakTargets.get(nd.getEnclosingLoop());
	...
	public Void visitWhileStmt(WhileStmt nd) {
		 ...
		 */
		NopStmt out = j.newNopStmt();
		NopStmt in = j.newNopStmt();

		Value cond = ExprCodeGenerator.generate(nd.getExpr(), fcg);

		breakTargets.put(nd, out);
		
		units.add(in);
		units.add(j.newIfStmt(j.newEqExpr(cond, IntConstant.v(0)), out));
		nd.getBody().accept(this);
		units.add(j.newGotoStmt(in));
		units.add(out);
		return null;
	}
}
